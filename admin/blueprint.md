...
The gate uses the `admin` role from the baseline RBAC shipped by `auth-jwt`. Without this block, consumer projects can still use `requireRole(event, 'admin')` in their own handlers — this block just gives you a consistent admin UI scaffold to hang routes off of.
...
`app/middleware/admin.ts` runs `useAuth()` / `checkAuth()` and redirects the user to `/` if their `roles` array does not include `'admin'`. Apply it on any `/admin/*` page via `definePageMeta({ middleware: ['auth', 'admin'] })`.

Server-side guards live in `auth-jwt`'s `server/utils/rbac.ts` — use `await requireRole(event, 'admin')` or `await requirePermission(event, 'x.y')` at the top of any admin API handler.
