...
const availableRoles = Object.values(ROLES) as RoleDefinition[]
  ...
  <div>
    ...
    <div class="border border-(--ui-border) rounded-lg overflow-hidden bg-(--ui-bg-elevated)">
      <UTable
        ...
        :meta="tableMeta"
        :ui="{ tr: 'transition-colors cursor-pointer' }"
      />
    </div>
          ...
          <section>
            ...
            <div class="space-y-2">
              <label
                ...
                :key="role.name"
                class="flex items-start gap-3 p-3 rounded-lg border border-(--ui-border) hover:bg-(--ui-bg-accented) cursor-pointer transition-colors"
              >
                <UCheckbox
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-sm text-(--ui-text-muted) mt-0.5">{{ role.description }}</div>
                </div>
              </label>
            </div>
